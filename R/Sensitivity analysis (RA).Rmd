...
```{r}
rau <- svy_uv.logit(design = nhs,
             y = 'arghritis_typera',
             x = c('microbe'))
write.csv(rau,file = 'rau.csv')
...
```{r}
ram1 <- svyglm(arghritis_typera ~ microbe + age + sex ,nhs,
...
```{r}
ram2 <- svyglm(arghritis_typera ~ microbe + age + sex + bminew + smoke + alcohol,nhs,
       family = quasibinomial()) |>
    reg_table()
...
```{r}
ram3 <- svyglm(arghritis_typera ~ microbe + age + sex + edu + povertynew + eth + marital+ bminew + smoke + alcohol ,nhs,
       family = quasibinomial()) |>
    reg_table()
...
```{r}
ram4 <- svyglm(arghritis_typera ~ microbe + age + sex + edu + povertynew + eth + marital + bminew + smoke + alcohol+ medQ + aheiQ + diiQ + heiQ+neutrophil+wbc ,nhs,family = quasibinomial()) |>
    reg_table()
